...
- 작업이 요구하는 것 이상으로 기능·리팩터링·추상화를 추가하지 않는다. 버그 수정에 주변 정리를 끼워 넣지 않고, 한 번 쓰는 처리에 헬퍼를 만들지 않는다.
- 가상의 미래 요구를 위해 설계하지 않는다. 잘 동작하는 가장 단순한 방법을 택하고 미완성 구현을 남기지 않는다.
- 일어날 수 없는 경우를 위한 에러 처리·폴백·검증을 넣지 않는다. 내부 코드와 프레임워크의 보장은 신뢰하고, 검증은 시스템 경계(사용자 입력, 외부 API, 외부 호출)에서 한다.
- 코드를 그냥 바꾸면 되는 자리에 기능 플래그나 하위 호환 shim을 만들지 않는다.
