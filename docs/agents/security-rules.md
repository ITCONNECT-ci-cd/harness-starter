...
- 조직·계정의 전체 저장소 목록, 프로젝트별 운영 여부·담당자·업데이트 판단표, 해당 조사 결과와 원시 자료는 비공개 지식 저장소에서 관리하며 이 Git 저장소에 커밋·푸시하지 않는다.
- 공통 하네스 문서에는 일반적인 설정·사용법·검증 기준만 남긴다. 실제 프로젝트명·브랜치·커밋을 모은 내부 목록을 예시나 변경 상세에 포함하지 않는다.
- Git에서 제외한 내부 자료를 이름이나 경로를 바꿔 다시 올리지 않는다. 요약 문서를 작성할 때에도 내부 프로젝트 목록이 섞였는지 확인한다.
...
방어 목적의 보안 리뷰·수정도 모델의 안전 분류기를 건드려 요청이 거부될 수 있다. 거부가 발생하면:

- 같은 요청을 그대로 재시도하지 않는다. 거부는 일시적 오류가 아니다.
- 대상과 목적(이 저장소 코드의 방어적 점검이며 공격 도구 제작이 아님)을 명시해 범위를 좁혀 다시 요청하거나, 해당 항목을 사람 검토로 넘긴다.
- 거부로 확인하지 못한 항목은 통과가 아니라 미검증으로 기록한다. `REVIEW.md`의 보안 항목을 근거 없이 APPROVED로 판정하지 않는다.
- 자동 파이프라인에서는 폴백 모델 설정 여부를 확인하고, 폴백이 없으면 해당 단계를 실패로 남긴다.
