...
- 환경변수: 테스트에서 환경변수를 직접 수정하지 않음 (`process.env.X = ...`, `os.environ[...] = ...`). 프레임워크의 격리 도구(`vi.stubEnv`, pytest `monkeypatch.setenv`) 또는 설정 주입 사용. **반대로 환경변수가 특정 상태(예: 미설정)라고 단언하는 테스트도 만들지 않음** — 다른 테스트가 남긴 값에 따라 결과가 바뀐다. 필요하면 그 테스트 안에서 격리 도구로 상태를 명시적으로 만든다
...
전체 실행에서만 실패하거나 실행할 때마다 결과가 달라지는 테스트는 아래 순서로 다룹니다.

1. **단독 실행으로 판별한다.** 단독 통과 + 전체 실패면 격리 문제다. 타임아웃이나 성능 문제로 단정하지 않는다.
2. **대기 시간을 늘려 넘어가지 않는다.** `waitFor` 타임아웃이나 재시도 횟수를 올리는 수정은 실제 결함을 덮는다. 위 격리 규칙 위반을 먼저 찾는다.
3. **실패한 실행의 로그를 근거로 쓴다.** `state/validate/<실행 디렉터리>/*.log`는 실행마다 보존된다. ad-hoc으로 다시 돌려 재현을 시도하기 전에 그 로그부터 읽는다.
4. **재현하지 못하면 미확정으로 남긴다.** 시도한 방법과 횟수를 기록하고, 재현되지 않은 수정을 원인으로 보고하지 않는다. 그 수정이 독립적으로 옳다면 수정은 남기되 인과가 입증되지 않았음을 함께 적는다.
5. **아무것도 증명하지 못하는 재현 테스트는 남기지 않는다.** 통과하든 실패하든 결론이 같은 테스트는 삭제한다. 남기려면 그 테스트가 대상 경로를 실제로 실행하는지 먼저 확인한다.
